...
14. **GDScript-Lambdas fangen lokale Variablen NACH WERT, nicht nach Referenz.**
    `var done := false; some_signal.connect(func(): done = true)` setzt danach NIE die
    äußere `done` — die Lambda schreibt in ihre eigene Kopie. In Tests deshalb auf einen
    tatsächlichen Zustand pollen (z. B. `BattleManager.in_battle`), keinen lokalen
    Flag über eine Lambda setzen lassen. Hat beim EXP-Balken-Test (Modul P) eine
    Kampf-Warteschleife scheinbar "ewig hängen" lassen, bis klar war, dass `finished`
    nie ankam.
15. **`BattleManager._screen` ist NICHT gleich "bereit".** `BattleManager.in_battle` wird
    ganz am Anfang von `_begin()` synchron auf `true` gesetzt, `_screen` wird direkt danach
    synchron instanziert – beides also schon im selben Frame wahr/gesetzt, BEVOR die Node
    ueberhaupt im Baum ist (`_ready()` noch nicht gelaufen, `@onready`-Felder noch null) und
    bevor `start()` aufgerufen wurde. Ein Test, der nur auf `in_battle` oder `_screen != null`
    pollt und dann sofort `_screen._choose_move()` aufruft, crasht mit "Cannot call method
    'hide' on a null value" (irgendein `%UniqueNode`-Onready-Feld). Fix: nach dem Auftreten
    von `_screen` noch 2-3 `await get_tree().process_frame` abwarten (genau wie `_begin()`
    selbst intern zwei Frames abwartet, bevor es `start()` aufruft), erst DANACH auf
    `screen._busy` pollen.
16. **CanvasLayer-`layer`-Werte: höher = weiter VORNE, nicht weiter hinten.** Battle liegt
    auf `layer=20`, die `DialogueBox` (DialogueManager) nur auf `layer=15` — ruft man
    `DialogueManager.run()/.ask()` waehrend die Battle-CanvasLayer offen ist, wird der
    Dialogtext unsichtbar HINTER dem Kampfbildschirm gezeichnet. Deshalb haben sowohl das
    "Attacke vergessen?"-Menü (Modul Q) als auch der `EvolutionScreen` (Modul R) eine
    EIGENE Text-/Auswahl-UI statt DialogueManager zu benutzen — der EvolutionScreen liegt
    zusätzlich bewusst auf `layer=25` (über Battle), damit er auch als eigenständiger
    Vollbild-Screen mitten im Kampf sichtbar bleibt.
...
- **Entwicklungssteine / Tausch-Entwicklung** — `EvolutionManager` (Modul R) kann bisher nur
  den `"level-up"`-Trigger aus `evolutions.json`. `"use-item"` (z. B. Feuerstein) und
  `"trade"` (z. B. Kadabra→Alakazam) sind in den Daten vorhanden, aber es gibt weder
  Stein-Items in `items.json` noch eine Tausch-Funktion. Wäre eine kleine Erweiterung nach
  demselben Muster wie die VM-Anwendung im Beutel (Modul Q).
...
| P | EXP-Balken im Kampf verdrahtet (dünner blauer Balken unter Spieler-HP, wie im Original – als schlichter ProgressBar/StyleBoxFlat statt Sprite-Zuschnitt, konsistent mit dem HP-Balken) |
| Q | Attacken-Lernsystem: Level-Up lernt automatisch neue Attacken aus dem `learnset` (mit "Attacke vergessen?"-Menü bei vollem Moveset, im Kampf), VMs/TMs (`category: "tm"` + `teaches`-Feld in `items.json`) lassen sich im Beutel-Menü einem Pokémon beibringen. Erstes Item: VM Zertrümmerer (`rock-smash`, neu in `moves.json`). `smash_rock.gd` prüft jetzt echt, ob ein Team-Pokémon die Attacke kennt (nicht nur Beutel-Besitz) |
| R | Level-Up-Entwicklung mit voller Animation (`EvolutionManager`-Autoload + `EvolutionScreen`: Blitz-Flackern, Sprite-Tausch, Einflug, Schrei, "Entwicklung abbrechen?"-Abfrage) – nutzt `GameData.evolution_target()`, das schon vorhanden war, aber nie aufgerufen wurde. Trefferblitz je nach Attacken-Typ eingefärbt (`hit_effect.gd`, rein prozedural per `_draw()`, kein Sprite-Asset) |
